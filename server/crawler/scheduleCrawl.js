...
const crawlStatusSchedule = async(date) => {
    ...
    try {
        ...
        // Tính toán ngày kết thúc bằng cách thêm 7 giờ vào ngày và giờ hiện tại
        const endDateTime = new Date(currentDate.getTime() + 7 * 60 * 60 * 1000);
        // Chuyển đổi ngày và giờ hiện tại và ngày kết thúc sang chuỗi ISO 8601
        const startTimeStamp = currentDate.toISOString();
        const endTimeStamp = endDateTime.toISOString();

        await updateScheduleByTime(startTimeStamp, endTimeStamp, arrMatch);
    } catch (error) {
        console.error("Error:", error);
        throw new Error("Internal server error");
    }
